...
for method in ['kmeans', 'random']:
    ...
    train_dir = os.path.join(f"data_{method}_sample", "train")
    val_dir = os.path.join(f"data_{method}_sample", "val")
    ...
    torch.save(model_ft.state_dict(), f"trained_model_{method}.pt")

    # Basic visualizations of the model performance
    fig = plt.figure(figsize=(20,10))
    plt.title(f"Train - Validation Loss, Method: {method}")
    plt.plot( loss_dict['train'], label='train')
    plt.plot(  loss_dict['val'], label='validation')
    plt.xlabel('num_epochs', fontsize=12)
    plt.ylabel('loss', fontsize=12)
    plt.legend(loc='best')
    plt.savefig(f'train_val_loss_plot_{method}.png')
    ...
    plt.legend(loc='best')
    plt.savefig(f'train_val_acc_plot_{method}.png')